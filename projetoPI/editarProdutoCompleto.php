...
<body>
  ...
  $cmd = $pdo->query("SELECT P.CATEGORIA_ID,C.CATEGORIA_ID ,C.CATEGORIA_NOME,PQ.PRODUTO_QTD
   FROM 
    ...
    <div class="row">
      ...
      function semLetra(a){
        ...
        if ((x>=48 && x<=57) || (x==44))
        {
          ...
          <div class="input-group">
          <input type="number" class="form-control" name="qtdEstoque" min="0" onkeypress="return semLetra(event)" aria-describedby="emailHelp" value="<?php echo $qtd?>" placeholder="digite a quantidade"  required>
  <script type="text/javascript">
      function semLetra(a){
        var x= a.which || e.keycode;
        if ((x>=48 && x<=57))
